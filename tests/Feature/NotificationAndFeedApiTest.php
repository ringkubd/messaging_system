...
class NotificationAndFeedApiTest extends TestCase
{
    ...
    public function test_message_send_creates_in_app_notification_for_recipient(): void
    {
        ...
        $this->assertSame('new_message', $notificationList[0]['data']['kind']);
    }
    ...
    public function test_user_can_create_image_only_post(): void
    {
        ...
        $this->assertSame('', $response->json('body'));
    }

    public function test_post_creation_is_idempotent_with_same_request_key(): void
    {
        $user = User::factory()->create();
        Sanctum::actingAs($user);

        $headers = ['X-Idempotency-Key' => 'post-123'];

        $first = $this->withHeaders($headers)->postJson('/api/v1/posts', [
            'body' => 'Only create this once',
        ])->assertCreated();

        $second = $this->withHeaders($headers)->postJson('/api/v1/posts', [
            'body' => 'Only create this once',
        ])->assertCreated();

        $this->assertSame($first->json('id'), $second->json('id'));
        $this->assertSame(1, (int) \App\Models\Post::query()->count());
    }
}
