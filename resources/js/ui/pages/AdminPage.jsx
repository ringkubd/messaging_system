                        ...
                        <tbody>
                            {list.map((p) => (
                                <tr key={p.id}>
                                    ...
                                    <td><Badge variant={PLACEMENT_STATUS_VARIANTS[p.status] || 'default'}>{STATUS_LABELS[p.status] || p.status}</Badge></td>
                                    <td>
                                        <div className="admin-actions">
...
function AdminLiveStreams() {
    ...
    const { loading, error, data, unauthorized, reload, pagination } = useApiData(`/api/v1/live-streams?${params.toString()}`);
    const list = Array.isArray(data) ? data : [];
    ...
    return (
        <div>
            ...
            {list.length === 0 ? (
                <EmptyState icon="📺" sub="No live streams found.">No streams</EmptyState>
            ) : (
                <div className="admin-table-wrap">
                    <table className="admin-table">
                        <thead>
                        ...
                        <tbody>
                            {list.map((s) => (
                                <tr key={s.id}>
                                    ...
                                    <td className="text-sm">{s.creator?.name || 'Unknown'}</td>
                                    <td>
                                        <Badge variant={STREAM_STATUS_VARIANTS[s.status] || 'default'}>
                                    ...
                                    <td className="text-sm text-muted">{s.scheduled_at ? new Date(s.scheduled_at).toLocaleDateString() : '-'}</td>
                                    <td className="text-sm text-muted">{s.started_at ? new Date(s.started_at).toLocaleDateString() : '-'}</td>
                                    <td className="text-sm text-muted">{s.ended_at ? new Date(s.ended_at).toLocaleDateString() : '-'}</td>
                                    <td className="text-sm">{s.event?.title || '-'}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
