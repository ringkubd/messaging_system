...
export default function ChatsPage({ user }) {
    ...
    const groupsState = useApiData('/api/v1/groups');
    const [selectedGroup, setSelectedGroup] = React.useState(null);
    const [showCreateGroup, setShowCreateGroup] = React.useState(false);
    const [groupDetail, setGroupDetail] = React.useState(null);

    const convList = data && Array.isArray(data) ? data : [];
    const groupsList = groupsState.data && Array.isArray(groupsState.data) ? groupsState.data : [];
    const activeConv = activeTab === 'chats' ? convList.find((c) => c.id === selectedConv) : null;
    const activeGroup = activeTab === 'groups' ? groupsList.find((g) => g.id === selectedGroup) : null;
    ...
    React.useEffect(() => {
        ...
        async function loadDetail() {
            try {
                const res = await window.axios.get(`/api/v1/groups/${selectedGroup}`);
    ...
    function handleSelectGroup(id) {
        setSelectedGroup(id);
        if (activeConv) setSelectedConv(null);
    }

    function handleGroupCreated(newGroup) {
        groupsState.prepend(newGroup);
    ...
    async function toggleGroupMute() {
        ...
        try {
            await window.axios.post(`/api/v1/groups/${selectedGroup}/mute`);
            setGroupDetail((prev) => prev ? { ...prev, is_muted: !prev.is_muted } : null);
            groupsState.update(selectedGroup, (item) => ({ ...item, is_muted: !item.is_muted }));
            toast.success(groupDetail?.is_muted ? 'Group unmuted.' : 'Group muted.');
        } catch (err) {
            toast.error(err?.response?.data?.message || 'Could not toggle mute.');
        }
    }
                ...
                <div className="chats-list">
                    <div className="chats-list-header">
                        <div className="tabs" style={{ marginBottom: '0.5rem' }}>
                            <button className={`tab ${activeTab === 'chats' ? 'active' : ''}`} onClick={() => { setActiveTab('chats'); setSelectedGroup(null); }} type="button">Chats</button>
                            <button className={`tab ${activeTab === 'groups' ? 'active' : ''}`} onClick={() => { setActiveTab('groups'); setSelectedConv(null); }} type="button">Groups</button>
                        </div>
                        {activeTab === 'chats' && (
                            <>
                                <input
                        ...
                        {activeTab === 'groups' && (
                            <button className="btn btn-primary btn-sm" style={{ width: '100%' }} type="button" onClick={() => setShowCreateGroup(true)}>
                                + Create Group
                            </button>
                        )}
                    </div>
                    ...
                    {activeTab === 'groups' && (
                        groupsState.loading ? <Spinner /> :
                        groupsList.length === 0 ? (
                            <div className="chats-empty">No groups yet. Create one!</div>
                        ) : (
                            groupsList.map((grp) => {
                                const memberCount = grp.members_count || grp.members?.length || 0;
                                return (
                                    <div
                                        key={grp.id}
                                        className={`chats-conversation ${selectedGroup === grp.id ? 'active' : ''}`}
                                        onClick={() => handleSelectGroup(grp.id)}
                                    >
                                        <Avatar name={grp.name} size="sm" />
                                        <div className="chats-conversation-info">
                                            <div className="chats-conversation-name">{grp.name}</div>
                                            <div className="chats-conversation-preview">
                                        ...
                                        <div className="chats-conversation-right">
                                            <div className="chats-conversation-time">{grp.is_private ? '🔒' : '🌐'}</div>
                                        </div>
                                    </div>
                                );
                            })
                ...
                <div className="chats-window">
                    {activeTab === 'groups' && activeGroup ? (
                        <GroupChatWindow
                            ...
                            onMuteToggle={toggleGroupMute}
                            onMembersAdded={() => {
                                if (selectedGroup) {
                                    window.axios.get(`/api/v1/groups/${selectedGroup}`).then((res) => {
                                        setGroupDetail(res.data);
                                        groupsState.update(selectedGroup, (item) => ({ ...item, members_count: res.data.members?.length || item.members_count }));
                                    }).catch(() => {});
                                }
                            }}
                        />
                        ...
                        <div className="chats-empty">
                            ...
                            {activeTab === 'groups' ? 'Select a group to start chatting' : 'Select a conversation or search for someone to start chatting'}
                        </div>
                    ) : (
                        <>
